...
Autonomous OpenShift Virtualization maintenance assurance operator built with Kubebuilder (`example.com/drift-operator`). Validates platform health — ClusterVersion stability, MachineConfigPool rollouts, KubeVirt/CDI/SSP/HyperConverged operator status, virt-handler readiness, VirtualMachineInstanceMigration phase tracking, NodeMaintenance detection — during CHG maintenance windows. Each cluster runs an independent operator instance that consumes Kafka `CHG_INITIATED` events, evaluates 11 safety gates locally, and publishes deterministic validation reports to Kafka.
...
├── api/
│   └── v1alpha1/                  # CRD Schemas (ChangeWindow, ClusterAppReport, PropagationStatus)
...
│   ├── SRE_AGENT_ARCHITECTURE.md  # Autonomous Federated Peer-to-Peer Topology & LangGraph Workflow
...
- **Native OpenShift Virtualization & Platform Resource Validation**:
  - `ClusterVersion`: Validates OCP version stability (not progressing, Available=True).
...
- **[SRE_AGENT_ARCHITECTURE.md](docs/SRE_AGENT_ARCHITECTURE.md)** — Autonomous Federated Peer-to-Peer Topology, SRE Agent design, GitHub PR/Tag API integration, & LangGraph state graph.
